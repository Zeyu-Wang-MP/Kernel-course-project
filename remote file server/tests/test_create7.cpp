...
/*
    Tests creating in sparse direntry space with size of directory > 1
    (specifically tests bug in fs_create() where I stored the wrong array of direntries
     in memory for modification)
*/
...

{
    ...
    fs_create("user1", "password1", session, seq++, "/dir/file9", 'f'); // /dir is now size = 2
    fs_create("user1", "password1", session, seq++, "/dir/file10", 'f');

    fs_delete("user1", "password1", session, seq++, "/dir/file5"); // delete in first block

    fs_create("user1", "password1", session, seq++, "/dir/file5", 'f'); // create again
}