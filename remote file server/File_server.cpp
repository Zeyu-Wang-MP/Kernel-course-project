...
// Parent directories must exist to create file/directory @2276
unsigned File_server::get_inodeblock(const string &pathname, const string &user)
{
    // Special case
    if(pathname == "/") return 0;

    // Vector of directory strings 
    std::vector<string> parsed_path = parse_path(pathname);

    // Initialize variables
    fs_inode parent_inode;
    string prev_path = "/";
    mutex* prev_path_lock = nullptr;
    mutex* curr_path_lock = nullptr;

    // Safely acquire lock for root directory
    {
        std::lock_guard<mutex> raii_lock(file_server_lock);
        prev_path_lock = &blocks_locks[prev_path]; 
    }
    prev_path_lock->lock();
    disk_readblock(0, &parent_inode);

    // Find inode for each parent directory until target inode found.
    for (auto iter = parsed_path.begin(); iter != parsed_path.end(); ++iter)
    {
        string directory = *iter;

        // Check user permissions
        if(prev_path != "/" && parent_inode.owner != user)
        {
            prev_path_lock->unlock();
            throw runtime_error("get_fileblock: Invalid user permissions");
        }
        ...
        {
            prev_path_lock->unlock();
            throw runtime_error("get_fileblock: Invalid directory (file treated as directory)");
        }
        
        // hand-over-hand locking
        {
            std::lock_guard<mutex> raii_lock(file_server_lock);
            if(prev_path == "/") curr_path_lock = &blocks_locks[prev_path + directory];
            else                 curr_path_lock = &blocks_locks[prev_path + "/" + directory];
        }

        curr_path_lock->lock();
        if(curr_path_lock != nullptr) prev_path_lock->unlock(); // TODO: what if it is nullptr?

        // Iterate through direntries to find inode for 'directory' (i.e. next parent)
        for (unsigned i = 0; i < parent_inode.size; ++i)
        {
            // Read direntries in block to table
            fs_direntry block_direntries[FS_DIRENTRIES];
            disk_readblock(parent_inode.blocks[i], block_direntries);

            for (unsigned j = 0; j < FS_DIRENTRIES; ++ j)
            {
                // ignore if dir entry not used
                if(block_direntries[j].inode_block == 0) continue;

                // ignore if not target inode
                if(block_direntries[j].name != directory) continue;
                
                // do not make an extra read if last directory.
                if(iter == parsed_path.end() - 1) 
                {
                    curr_path_lock->unlock();
                    return block_direntries[j].inode_block;
                }

                disk_readblock(block_direntries[j].inode_block, &parent_inode);
            }
        }
        
        // Change curr_path lock to prev path
        if(prev_path == "/") prev_path = prev_path + directory;
        else prev_path = prev_path + "/" + directory;
        prev_path_lock = curr_path_lock;
    }

    throw runtime_error("DEBUG: get_fileblock did not find path. Check if valid path before passing in.");
    return 0;
}
